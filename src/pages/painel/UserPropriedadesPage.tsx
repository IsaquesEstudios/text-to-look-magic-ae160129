...
function aggregateByType(shares: any[], type: string): AggregatedProperty[] {
  ...
  shares.forEach((s) => {
    ...
    if (existing) {
      ...
      map.set(prop.id, { prop, totalQuantity: s.quantity, totalPaid: Number(s.amount_paid) });
    }
  });
...
function KPICards({ aggregated }: { aggregated: AggregatedProperty[] }) {
...
function PropertyGrid({ aggregated, emptyIcon: EmptyIcon, emptyText }: { aggregated: AggregatedProperty[]; emptyIcon: any; emptyText: string }) {
  ...
  if (!sorted.length) {
    return (
      <div className="rounded-2xl border border-dashed border-border/40 flex flex-col items-center justify-center py-20 text-muted-foreground">
        <EmptyIcon className="h-9 w-9 mb-3 opacity-25" />
            ...
            <div className="aspect-[16/10] bg-secondary/50 overflow-hidden">
              {prop.cover_image_url ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <EmptyIcon className="h-8 w-8 text-muted-foreground/20" />
                </div>
              )}
            </div>
...
export default function UserPropriedadesPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-foreground tracking-tight">Meus Projetos</h1>
        <p className="text-sm text-muted-foreground mt-1">Imóveis e terrenos em que você investe</p>
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="bg-secondary/50 border-0 p-1 rounded-xl h-auto">
          ...
          >
            ...
            {houses.length > 0 && (
              <>
                ...
                <span className="text-xs text-muted-foreground">{houses.length}</span>
              </>
            )}
          </TabsTrigger>
          <TabsTrigger
          ...
          >
            ...
            {lands.length > 0 && (
              <>
                <span className="w-px h-3.5 bg-border/60 mx-1" />
                <span className="text-xs text-muted-foreground">{lands.length}</span>
              </>
            )}
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="house" className="mt-6 space-y-6">
          <KPICards aggregated={houses} />
          <PropertyGrid aggregated={houses} emptyIcon={Building2} emptyText="Você ainda não possui imóveis vinculados" />
        </TabsContent>

        <TabsContent value="land" className="mt-6 space-y-6">
          <KPICards aggregated={lands} />
          <PropertyGrid aggregated={lands} emptyIcon={MapPin} emptyText="Você ainda não possui terrenos vinculados" />
        </TabsContent>
      </Tabs>
    </div>
  );
